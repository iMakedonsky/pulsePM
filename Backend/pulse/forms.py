...
from pulse.models import Member, WorkItem

Choice = tuple[str, str]


def get_choices_for_account() -> list[Choice]:
    return [('', '--blank--')] + [(str(account.pk), account.username) for account in User.objects.all()]


def get_choices_for_member() -> list[Choice]:
    return [('', '--blank--')] + [(str(member.pk), member.user.username) for member in Member.objects.all()]
...
class AddItemForm(forms.Form):
    ...
    due_date = forms.DateField(
        widget=forms.DateInput(attrs={'type': 'date', 'clase': 'form-control'}),
        input_formats=['%Y-%m-%d'],
    )
