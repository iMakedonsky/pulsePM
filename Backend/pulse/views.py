    ...
    def get(self, request: HttpRequest, *args: Any, **kwargs: Any) -> HttpResponse:
        return render(
            ...
            'pulse/organization.html',
            {
                ...
                'workspace_list': WorkSpace.objects.filter(organization_id=kwargs['organization_id']),
                'member_list': Member.objects.filter(
                    organization_id=kwargs['organization_id'],
                    role__in=['contributor', 'project-manager'],
                ),
                'owner': Member.objects.get(organization_id=kwargs['organization_id'], role='owner'),
            },
        )
        ...
        if form.is_valid():
            try:
                ...
                new_workitem = WorkItem.objects.create(
                    ...
                    created_by=Member.objects.get_or_create(user=user, organization_id=org_id)[0],
